...
radius=90+1:1:150;%for radius range for unknown radius 3D
...
for y=1:height
    for x=1:width
        if myimage(y,x)>0%for each edge pixel
            %for z=1:length(radius)%for unknown radius
                %r=radius(z);%for unknown radius
                r=100;%for static radius
...
figure(2)
imshow(imadjust(rescale(accumulator)),[],'XData',a_axis,'YData',b_axis,'InitialMagnification','fit')
xlabel('a_axis')
ylabel('b_axis')
title('accumulator')
axis on
